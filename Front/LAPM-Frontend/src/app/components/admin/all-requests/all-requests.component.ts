...
export class AllRequestsComponent implements OnInit {
  ...
  revoke(id: number): void {
    if (confirm('Are you sure you want to revoke this grant immediately?')) {
      this.adminService.revokeRequest(id).subscribe(() => this.refreshSubject.next());
    }
  }
  ...
  extend(id: number): void {
    ...
    if (hours && !isNaN(+hours)) {
      ...
      this.adminService.extendRequest(id, newExpirationTime).subscribe(() => this.refreshSubject.next());
    }
  }
}
